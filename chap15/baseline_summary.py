...
baseline_prompt = f"""다음 텍스트를 간결하게 요약해 주십시오:
...
baseline_generation_config = GenerationConfig(
    temperature=0.2,
    max_output_tokens=150 # 요약 길이에 맞게 조절
)
...
try:
    ...
    baseline_summary_text = baseline_response.text
    print("----- 기본 요약 -----")
    print(baseline_summary_text)
except Exception as e:
    print(f"기본 요약 생성 중 오류 발생: {e}")
    baseline_summary_text = "오류로 인해 요약을 생성할 수 없습니다."
